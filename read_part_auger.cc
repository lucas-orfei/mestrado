...
const int numDetectors = 1600;
const double tankRadius = 1.8; 
...
int main()
{
    ...
    strcpy(str_file_out, "./tratados_auger/photon/photon1E14_auger");
...
//    }
    ...
    int index = 0;
    int rows = (int)ceil(sqrt(numDetectors / 2.0));
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < rows; ++j) {
            if (index >= numDetectors) break;

            double x_offset = (i % 2 == 0) ? 0.0 : spacing / 2.0;
            x_det[index] = j * spacing + x_offset;
            y_det[index] = i * spacing * sqrt(3) / 2.0;
            ++index;
        }
    }
                ...
                if (idpart < 70) {
                    ...
                    if ((idpart >= 2) && (idpart <= 6)) {
                        ...
                        for (int i = 0; i < numDetectors; ++i) {
                            double dx = x / 100 - x_det[i];
                            double dy = y / 100 - y_det[i];
